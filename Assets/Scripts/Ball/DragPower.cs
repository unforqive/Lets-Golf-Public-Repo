...
public class DragPower : MonoBehaviour
{
    ...
    //public PhysicMaterial outOfBounds;
    ...
    void Start()
    {
        ...
        ball.sleepThreshold = 0.5f; //default is 0.005f;
        score.GetComponent<TMPro.TMP_Text>().text = strokes.ToString();
    ...
    void Update()
    {
        if (strokes == maxStrokesHandler.maxStrokes)
        {
            Debug.Log("stop playing");
            //stop player from shooting
        }

        if (ball != null && !pause.pause && Time.timeScale == 1)
        {
